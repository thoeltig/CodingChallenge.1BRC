				...
				Parallel.For(0, maxParallel, new ParallelOptions {
					...
					fixed (byte* blockPtr = block) {
						for (var i = 0; i < rowCount; i++) {
							...
							Marshal.Copy(name, 0, IntPtr.Add(ptr, blockIndex), nameLength);
							blockIndex += nameLength;

							const byte lineSeparator = 59; // ;
							blockPtr[blockIndex] = lineSeparator;
							blockIndex++;
							...
							if ((singleDigit != zeroByte || useDoubleDigit) && random.Next(2) == 0) {
								...
								blockPtr[blockIndex] = signedSymbol;
								blockIndex++;
							}

							if (useDoubleDigit) {
								blockPtr[blockIndex] = doubleDigit;
								blockIndex++;
							}

							blockPtr[blockIndex] = singleDigit;
							blockIndex++;

							if (fractionValue != zeroByte) {
								const byte temperatureSeparator = 46; // .
								blockPtr[blockIndex] = temperatureSeparator;
								blockIndex++;
								blockPtr[blockIndex] = fractionValue;
								blockIndex++;
							}

							// Write new line to block
							const byte newLine = 10; // \n
							blockPtr[blockIndex] = newLine;
							blockIndex++;
